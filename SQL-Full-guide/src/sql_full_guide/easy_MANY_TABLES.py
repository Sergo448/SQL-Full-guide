# -*- coding: utf-8 -*-
# Sergey.6362@mail.ru
# SQL-Full-guide/src/sql_full_guide/easy_MANY_TABLES.py
...
"""         LEFT and RIGTH JOIN
            ...
            LEFT OUTER JOIN - все строки из левой таблицы + связные строки из второй 
            RIGHT OUTER JOIN - все строки из правой + связные из левой
            NATURAL - устраняет необходимость явного указания имен связных столбцов
            USING - аналогично NATURAL и + все остальные условия когда столбцы не связные
            
            CROSS JOIN - перекрестное соединение (все возможные пары строк из двух таблиц)
            при таком типе соединения условия ON & USING не допускаются.
            
            UNION JOIN - расширенное соединение (нежелательное из-за выключения из современных стандартов)
            UNION - соединяет строки двух таблиц, у которых должно быть одинаковое число столбцов, 
            причем типы соответствующих столбцов должны быть совместимы.
...
quiery_28 = f"SELECT NAME, CITY " \
            f" FROM SALESREPS LEFT OUTER JOIN OFFICES " \
            f" ON  REP_OFFICE = OFFICE" \
            f" GROUP BY NAME;"
quiery_29 = f"SELECT NAME, CITY" \
            f" FROM OFFICES RIGHT OUTER JOIN SALESREPS " \
            f" ON OFFICE = REP_OFFICE " \
            f" GROUP BY NAME;"


# Делаем запрос к базе данных, используя обычный SQL-синтаксис
# cursor.execute(quiery_13)
cursor.execute(quiery_29)
